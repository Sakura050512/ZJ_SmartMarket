...
public class PointProductsServiceImpl extends ServiceImpl<PointProductsMapper, PointProducts> implements IPointProductsService {
    ...
    public Page<PointProducts> queryPageByQo(QueryPointProducts qo) {
        ...
        QueryWrapper<PointProducts> wrapper = new QueryWrapper<PointProducts>().like(StringUtils.hasText(qo.getName()), "goods_name", qo.getName()).eq("state", PointProducts.STATE_NORMAL);
    ...
    public List<Map<String, Object>> queryOptionGoods() {
        ...
        List<PointProducts> list = super.list(pointProductsQueryWrapper);
    ...
    public void savePointGoods(PointProducts pointProducts,String token) {
        ...
        QueryWrapper<PointProducts> wrapper = new QueryWrapper<PointProducts>()
            .eq("goods_id", pointProducts.getGoodsId())
            .eq("state", PointProducts.STATE_NORMAL);
    ...
    public void updatePointGoods(PointProducts pointProducts, String token) {
        ...
        // 先查询记录是否存在
        QueryWrapper<PointProducts> queryWrapper = new QueryWrapper<PointProducts>()
                .eq("goods_id", pointProducts.getGoodsId())
                .eq("state", PointProducts.STATE_NORMAL);
        PointProducts existingProduct = super.getOne(queryWrapper);
        if (existingProduct == null) {
            throw new BusinessException("积分商品不存在或已被删除");
        }
        
        // 更新记录
        UpdateWrapper<PointProducts> updateWrapper = new UpdateWrapper<PointProducts>()
                .set("integral", pointProducts.getIntegral())
                .eq("goods_id", pointProducts.getGoodsId())
                .eq("state", PointProducts.STATE_NORMAL);
